...
void solve() {
    int n; cin >> n;
    vector<int> A(n); for EACH(a, A) cin >> a; sort(A.begin(), A.end());
    int dp[sum(A) + 1] = {false}; dp[0] = true;
    for EACH(a, A) ROF(i, 0, sum(A) + 1) {
        if (i - a >= 0 && dp[i - a]) dp[i] = true;
    }
    int c = -1;
    for EACH(x, dp) if (x) c++;
    cout << c << '\n';
    FOR(i, 1, sum(A) + 1) if (dp[i]) cout << i << ' ';
}
