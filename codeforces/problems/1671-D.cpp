...
void solve() {
    ...
    ll s = 0; FOR(i, 1, n) s += abs(A[i] - A[i - 1]);
    int mi = min(A), ma = max(A); x = max(ma, x);
    int best = min(abs(A.back() - 1) + abs(A.front() - x), abs(A.back() - x) + abs(A.front() - 1));
    best = min(best, min(abs(A.back() - 1) + 2 * (x - ma), abs(A.front() - 1) + 2 * (x - ma)));
    best = min(best, min(abs(A.back() - x) + 2 * (mi - 1), abs(A.front() - x) + 2 * (mi - 1)));
    best = min(best, 2 * (mi - 1) + 2 * (x - ma));
    cout << s + best << '\n';
}
