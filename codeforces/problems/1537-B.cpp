...
void solve() {
    ...
    ll maxi = -1; p p1, p2;
    for (p x : {p(1, 1), p(1, m), p(n, m), p(n, 1)}) {
        for (p y : {p(1, 1), p(1, m), p(n, m), p(n, 1)}) {
            ll dist = 0LL + abs(x.first - i) + abs(x.second - j) + abs(x.first - y.first) + abs(x.second - y.second) + abs(y.first - i) + abs(y.second - j);
            if (dist > maxi) {
                maxi = dist;
                p1 = x; p2 = y;
            }
