...
void solve() {
    ...
    FOR(i, 0, n) {
        if (t[i] == '1') {
            if (i > 0 && taken[i - 1] == 0 && s[i - 1] == '1') taken[i - 1] = 1;
            else if (taken[i] == 0 && s[i] == '0') taken[i] = 1;
            else if (i < n - 1 && taken[i + 1] == 0 && s[i + 1] == '1') taken[i + 1] = 1;
        }
    }
    cout << accumulate(taken.begin(), taken.end(), 0) << '\n';
}
