...
void solve() {
    ...
    sort(pts.begin(), pts.end(), [] (t a, t b) {return a.second < b.second;});
    while (pts.size() > 2 * n) pts.pop_back();
    sort(pts.begin(), pts.end(), [] (t a, t b) {return a.first < b.first;});
    int weight = 0;
    for EACH(p, pts) weight += p.second;
    cout << weight << '\n';
    FOR(i, 0, n) {
        cout << pts[i].third << ' ' << pts[2 * n - 1 - i].third << '\n';
    }
}
