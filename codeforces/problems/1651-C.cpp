...
void solve() {
    int n; cin >> n;
    vector<ll> A(n), B(n); for EACH(a, A) cin >> a; for EACH(b, B) cin >> b;
    auto bd = [] (ll x, vector<ll> &V) {
        ll d = BIG32;
        for EACH(v, V) d = min(d, abs(x - v));
        return d;
    };
    ll best = min(abs(A.back() - B.back()), bd(A.back(), B) + bd(B.back(), A)) + min(abs(A.front() - B.front()), bd(A.front(), B) + bd(B.front(), A));
    best = min(best, min(abs(A.back() - B.front()), bd(A.back(), B) + bd(B.front(), A)) + min(abs(A.front() - B.back()), bd(A.front(), B) + bd(B.back(), A)));
    cout << best << '\n';
}
