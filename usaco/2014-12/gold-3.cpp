...
void solve() {
    int n, t; cin >> n >> t;
    vector<pl> A(n); for EACH(a, A) {
        cin >> a.first >> a.second;
        a.second = a.first + a.second * t;
    }
    vector<ll> V = {-A[0].second};
    FOR(i, 1, n) {
        int x = bisect_right(V, -A[i].second);
        if (x == V.size()) V.push_back(-A[i].second);
        else V[x] = -A[i].second;
    }
    cout << V.size();
}
